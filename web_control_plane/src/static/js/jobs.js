// Form Khởi tạo Job Ảnh
document.getElementById('jobForm')?.addEventListener('submit', async (e) => {
    e.preventDefault();
    const btn = document.getElementById('submitBtn');
    btn.disabled = true; btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span> Đang đẩy...';
    ...
    await fetch('/jobs/image', { method: 'POST', body: formData });
    e.target.reset(); fetchJobs();
...
async function fetchJobs() {
    try {
        ...
        jobs.forEach(job => {
            ...
            const trHtml = `<tr><td class="ps-4 tech-font text-muted">#${job.id}</td><td class="fw-bold text-light">${job.product_code}</td><td><span class="status-badge status-${job.status}"><span class="status-dot"></span>${job.status.toUpperCase()}</span></td><td class="tech-font text-muted">${timeStr}</td><td class="text-end pe-4">${actionBtn}</td></tr>`;
...
document.getElementById('btnTriggerVideo')?.addEventListener('click', async () => {
    ...
    await fetch(`/jobs/video/${activeImageJobId}`, { 
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ approved_indexes: validIndexes }) 
    });
    
    qaModal.hide(); fetchJobs();
    new bootstrap.Tab(document.getElementById('video-tab')).show();
});

// Logic Modal Xem Video
