...
// Cấu hình số vòng lặp tạo ảnh cho mỗi Job
const LOOP_COUNT = 3; 
...
async function processJob(job) {
    console.log(`\n==========================================`);
    console.log(`🚀 BẮT ĐẦU JOB ẢNH ID: ${job.id} | SP: ${job.product_code} | SỐ VÒNG: ${LOOP_COUNT}`);
    console.log(`==========================================`);
    ...
    try {
        ...
        for (let i = 1; i <= LOOP_COUNT; i++) {
            console.log(`\n------------------------------------------`);
            console.log(`🔄 ĐANG CHẠY VÒNG LẶP THỨ ${i}/${LOOP_COUNT}`);
            console.log(`------------------------------------------`);
...
async function workerLoop() {
    ...
    while (true) {
        try {
            // Lấy thêm concept_id để phục vụ prompt builder
